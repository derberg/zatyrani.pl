/**
 * Wilczy Półmaraton 2026 — "last days of May" price reminder
 *
 * Third touch in the price-increase sequence (after wilczy-price-reminder
 * and wilczy-final-reminder). Sends to registered-but-unpaid 21km/canicross
 * participants. From June 1st wpisowe rises 100 zł → 130 zł; this reminder
 * leans on "ostatnie dni maja" rather than a specific day count so the copy
 * stays accurate regardless of which day in late May it goes out.
 *
 * HOW TO USE:
 ...
 *   3. Run: node scripts/send-wilczy-last-days-reminder.js
 *
...
const CAMPAIGN_ID = "wilczy-last-days-reminder-may-2026";
...
function buildSubject(firstName) {
  return `Wilczy: ${firstName}, ostatnie dni za 100 zł`;
}
...
function buildHtml(firstName, gender, registrationId, totalAmount) {
  ...
  const urgencyBlock = `
      <div style="background-color: #fef2f2; border-left: 4px solid #dc2626; padding: 16px 20px; margin: 20px 0; border-radius: 4px;">
        <p style="margin: 0; color: #991b1b; font-size: 16px; font-weight: bold;">🚨 OSTATNIE DNI MAJA — wpisowe 100 zł!</p>
        <p style="margin: 8px 0 0 0; color: #7f1d1d; font-size: 15px;">Już <strong>1 czerwca</strong> wpisowe na bieg 21km i canicross wzrasta do <strong>130 zł</strong>. Opłać start do końca maja i zaoszczędź 30 zł.</p>
      </div>`;
