...
async function getPageDetails(token: string): Promise<string> {
  ...
  try {
    ...
    const data: PageDetailsResponse = (await response.json()) as PageDetailsResponse;
  ...
  try {
    ...
    for (const post of posts) {
      if (post.full_picture && post.created_time) {
        const folderName = new Date(post.created_time).toISOString().replace(/[^a-zA-Z0-9-]/g, "");
        const dirPath = path.join(__dirname, "public", "facebook", folderName);

        await fsPromises.mkdir(dirPath, { recursive: true });

        // Download the image and save it to the directory
        const imageFullPath = await downloadImage(post.full_picture, path.join(dirPath));
...
async function updateNews(newPosts: NewsItem[]): Promise<void> {
  ...
  const uniqueNewPosts = newPosts.filter(
    (newPost) => !newsData.some((existingPost: NewsItem) => existingPost.id === newPost.id),
  );
  ...
  await fsPromises.writeFile(newsFilePath, JSON.stringify(newsData, null, 2), "utf-8");
}
